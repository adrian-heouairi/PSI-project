...
\begin{description}
    \item [\small\mintinline{go}{getPeerPathHashMapRecursive(peerName string, hash []byte, path string, currentMap map[string][]byte)}\normalsize]
    ...
    \item [\mintinline{go}{writeBigFile(peerName string, datum datumTree, path string, depth int)}] Il s'agit d'une fonction récursive qui écrit un \textit{big file} sur disque : on itère sur les hashs enfants de \textit{datum} : on télécharge l'enfant, si c'est un \textit{chunk} on l'\textit{append} au fichier sur disque, si c'est un \textit{big file} on fait un appel récursif. Ceci effectue un parcours préfixe de l'arbre \textit{big file} qui va écrire les \textit{chunks} dans l'ordre.
